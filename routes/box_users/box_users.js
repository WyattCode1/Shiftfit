...
function _delete_user_box(req, res) {
	...
	global.connection.query("UPDATE user_box SET rol_id=1 WHERE user_id=$1 AND box_id=$2", [user_id, box_id], 'Updating admin_user setting is admin false', function(user_box, err) {
		if (err) {
			return res.status(500).send();
		} else {
...
function _add_user_box(req, res) {
	...
	async.waterfall([
		...
		function(callback) {
			if (!existing) {
				global.connection.query("INSERT INTO user_box (user_id, box_id, rol_id) VALUES ($1, $2, 4)", [user_id, box_id], 'Inserting new user_box', function(user_box, err) {
					if (err) {
						return callback(res.__('internal_server_error'));
					}
					user_list = user_box;
		...
		function(callback) {
			if (existing) {
				global.connection.query("UPDATE user_box SET rol_id=4 WHERE user_id=$1 AND box_id=$2", [user_id, box_id], 'updating existing user_box', function(user_box, err) {
					if (err) {
						return callback(res.__('internal_server_error'));
					}
					user_list = user_box;
